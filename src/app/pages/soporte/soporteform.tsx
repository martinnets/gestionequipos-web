...
export default function SoporteForm() {
  ...
  useEffect(() => {
    ...
    if (id && id !== "crea") {
      ...
      asignacionDataService.getById(id).then((res: any) => {
        setAsignacion(res);
        // Cargar la solicitud asociada
        if (res.id_solicitud) {
          solicitudDataService.getById(res.id_solicitud).then((solRes: any) => {
            setSolicitud(solRes);
          });
        }
      });
  ...
  const validarFormulario = (): boolean => {
    if (!asignacion.id_tecnico) {
      setAlert({ message: "Debe seleccionar un técnico", type: "error" });
      return false;
    }

    if (!asignacion.fecha_inicio) {
      setAlert({ message: "Debe ingresar la fecha de inicio", type: "error" });
      return false;
    }

    if (!asignacion.fecha_fin) {
      setAlert({ message: "Debe ingresar la fecha de fin", type: "error" });
      return false;
    }

    // Validar que fecha inicio sea menor que fecha fin
    if (asignacion.fecha_inicio && asignacion.fecha_fin) {
      if (new Date(asignacion.fecha_inicio) > new Date(asignacion.fecha_fin)) {
        setAlert({ message: "La fecha de inicio debe ser menor a la fecha de fin", type: "error" });
        return false;
      }
    }

    if (!asignacion.checklist || asignacion.checklist.length === 0) {
      setAlert({ message: "Debe agregar al menos un item de checklist", type: "error" });
      return false;
    }
  ...
  const handleSubmit = async (e: any) => {
    ...
    const confirmacion = window.confirm(
      `¿Está seguro de ${id === "crea" ? "crear" : "actualizar"} esta asignación de soporte?`
    );
    ...
    try {
      ...
      if (id === "crea") {
        await asignacionDataService.create(dataToSave);
        setAlert({ message: "✅ Asignación creada correctamente", type: "success" });
      } else {
        await asignacionDataService.update(id, dataToSave);
        setAlert({ message: "✅ Asignación actualizada correctamente", type: "success" });
      }

      setTimeout(() => {
        navigate("/asignacionsoporte");
      }, 1500);
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="alert alert-dark p-4 d-flex justify-content-between bg-dark text-light">
          <div>
            <h3 className="text-light">
              {id === "crea" ? "Nueva Asignación de Soporte" : "Editar Asignación de Soporte"}
            </h3>
            <span>Asignación de técnico y checklist para preparación de equipo</span>
          </div>
          <div>
            <Link to="/asignacionsoporte" className="btn btn-danger btn-sm me-2">
        ...
        <div className="card p-4">
          ...
          <div className="alert alert-info">
            ...
            <div className="row">
              <div className="col-md-3">
                <p className="mb-2">
                  ...
                  <span className="badge bg-dark fs-6 text-light">COD-00000001</span>
                </p>
              </div>
              <div className="col-md-3">
                <p className="mb-2">
            ...
            <div className="row mt-3">
              <div className="col-md-6">
                <p className="mb-0">
                  <strong>Tipo Equipo:</strong> Laptop
                </p>
              </div>
              <div className="col-md-6">
                <p className="mb-0">
                  <strong>Gama:</strong>Alta
                </p>
              </div>
            </div>
          </div>
                      ...
                      <tr key={item.id_item}>
                        ...
                        <td>
                          <span className={`badge ${
                            item.tipo_validacion === 'checkbox' ? 'text-light bg-info' : 'bg-warning'
                          }`}>
          ...
          {/* ------------------ Comentarios -------------------- */}
          <h5 className="mb-3 mt-4">
            <i className="fa fa-comment"></i> Comentarios e Instrucciones
          </h5>
          ...
          <div className="mb-3">
            <textarea 
              ...
              onChange={handleChange}
              rows={5}
              placeholder="Ingrese comentarios, instrucciones especiales o notas para el técnico..."
            />
            <small className="text-muted">Opcional - Información adicional para el técnico asignado</small>
          </div>
